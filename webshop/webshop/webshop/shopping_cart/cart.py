...
def place_order(pos_config=None):
	...
	# Use our enhanced conversion function that preserves taxes and shipping
	from webshop.pos_api import convert_quotation_to_sales_order_enhanced
	
	frappe.log_error(f"Converting quotation {quotation.name}", "Cart SO")
	
	try:
		sales_order = convert_quotation_to_sales_order_enhanced(quotation.name, submit_order=False)
		frappe.log_error(f"Enhanced conversion OK - SO {sales_order.name}", "Cart SO")
	except Exception as e:
		frappe.log_error(f"Enhanced failed: {str(e)[:80]}", "Cart SO")
		# Fallback to standard conversion if our enhanced version fails
...
@frappe.whitelist()
def recalculate_cart_taxes():
	"""Recalculate taxes for the current cart quotation"""
	try:
		quotation = _get_cart_quotation()
		...
		return {
			"success": True,
			"message": "Cart taxes recalculated",
			"total_taxes": quotation.total_taxes_and_charges,
			"grand_total": quotation.grand_total
		}
	except Exception as e:
		frappe.log_error(f"Error recalculating cart taxes: {str(e)}")
		return {
			"success": False,
			"error": str(e)
		}
...
def apply_cart_settings(party=None, quotation=None):
	...
	frappe.log_error(f"apply_cart_settings for {quotation.name}", "Cart Tax")
	...
	frappe.log_error(f"apply_cart_settings done - {len(quotation.taxes)} taxes", "Cart Tax")
...
def set_taxes(quotation, cart_settings):
	...
	# Ensure we have a tax template - if not, set a default one
	if not quotation.taxes_and_charges:
		# Try to find a default tax template
		default_tax_template = frappe.db.get_value("Sales Taxes and Charges Template", 
			{"is_default": 1}, "name")
		if not default_tax_template:
			# Look for common tax templates
			default_tax_template = frappe.db.get_value("Sales Taxes and Charges Template", 
				{"name": ["like", "%US ST%"]}, "name")
		if default_tax_template:
			quotation.taxes_and_charges = default_tax_template
			frappe.log_error(f"Set default tax template: {default_tax_template}", "Cart Tax")
	
	frappe.log_error(f"Final tax template: {quotation.taxes_and_charges}", "Cart Tax")
	
	# Preserve existing shipping charges before clearing taxes
	existing_shipping_taxes = []
	for tax in quotation.taxes:
		if tax.description and "Shipping" in tax.description:
			existing_shipping_taxes.append({
				"charge_type": tax.charge_type,
				"account_head": tax.account_head,
				"description": tax.description,
				"tax_amount": tax.tax_amount,
				"total": getattr(tax, 'total', tax.tax_amount)
			})
	
	frappe.log_error(f"Preserving {len(existing_shipping_taxes)} shipping taxes", "Cart Tax")
	frappe.log_error(f"Tax template: {quotation.taxes_and_charges}", "Cart Tax")
	
	# Clear table
	quotation.set("taxes", [])
	
	# Append taxes from template
	if quotation.taxes_and_charges:
		quotation.append_taxes_from_master()
		frappe.log_error(f"After master: {len(quotation.taxes)} taxes", "Cart Tax")
	else:
		frappe.log_error(f"No tax template found", "Cart Tax")
		
	quotation.append_taxes_from_item_tax_template()
	frappe.log_error(f"After item tax: {len(quotation.taxes)} taxes", "Cart Tax")
	
	# Restore shipping charges
	for shipping_tax in existing_shipping_taxes:
		quotation.append("taxes", shipping_tax)
		
	frappe.log_error(f"Final tax count: {len(quotation.taxes)}", "Cart Tax")
