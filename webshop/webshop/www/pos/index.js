...
class FencePOS {
    constructor() {
        ...
        // Dynamic style-to-filters mapping - only show filters for available variants
        this.styleFilters = {
            'Solid': ['Height', 'Color', 'Rail Type'],
            'Lattice': ['Height', 'Color', 'Rail Type', 'Lattice Type'],
        ...
        // Default item sort order matching brochure order
        this.brochureSortOrder = ['Solid', 'Lattice', 'Open Spindle Top', 'Closed Spindle Top', '3"  Open Picket', '1.5" Open Picket', 'Picket', 'Ranch Rail'];
                ...
                <div class="cart-item ${item.isBundle ? 'bundle-item' : ''}" data-item-code="${item.item_code}">
                    ...
                    <div class="cart-item-controls">
                        <div class="cart-item-qty">
                            ...
                            <span>${item.qty % 1 === 0 ? item.qty : item.qty.toFixed(1)}</span>
            ...
            if (items.length === 0) {
                return { success: false, error: 'No items to quote' };
                            ...
                            // Create Project Quote using fence_supply app
                                // Ensure customer is selected before creating quote
                if (!this.selectedCustomer || this.selectedCustomer.id === 'Administrator') {
                    this.showNotification('Please select a customer before creating a quote', 'error');
                    return;
                }
                
                // Generate quote name/label
                const quoteLabel = `${this.selectedCustomer.name} Fence - ${this.selectedCategory} ${this.selectedStyle}`;
                
                const projectQuoteData = {
                    doctype: 'Project Quote',
                    customer: this.selectedCustomer.id,
                    contact_person: this.selectedCustomer.name,
                    project_name: quoteLabel, // Add quote name/label field
    ...
    async clearCart() {
        try {
            ...
            // Ask for confirmation
            if (!confirm(`Are you sure you want to clear the cart? This will remove ${cartItems.length} item(s).`)) {
                return;
            }
            
            // Store current view state to maintain position
            ...
            // Refresh cart display and maintain current view
            await this.refreshCart();
            
            // Restore view state instead of full page reload
            if (currentView === 'category' && currentCategory) {
                this.currentView = currentView;
                this.selectedCategory = currentCategory;
                await this.showCategoryView(currentCategory);
            } else {
                this.currentView = 'category';
                await this.showCategoryView();
            }
    ...
    async changePriceList(newPriceList) {
        /**
         * Change the current price list and refresh all prices immediately
         */
        try {
            ...
            // Set cart price list and update prices immediately
            console.log(`🏷️ Setting cart price list to: ${newPriceList}`);
            await this.setCartPriceList(newPriceList);
            
            // Force immediate price update for all visible items
            await this.updateVisibleItemPrices(newPriceList);
    ...
    async updateVisibleItemPrices(priceList) {
        /**
         * Update prices for all currently visible items immediately
         */
        try {
            console.log(`🔄 Updating visible item prices for price list: ${priceList}`);
            
            // Get all visible item cards
            const itemCards = document.querySelectorAll('.item-card');
            
            for (const card of itemCards) {
                const itemCode = card.dataset.itemId;
                if (itemCode) {
                    try {
                        // Get updated price for this item
                        const priceResponse = await frappe.call({
                            method: 'webshop.webshop.pos_api.get_item_price',
                            args: {
                                item_code: itemCode,
                                price_list: priceList
                            }
                        ...
                        if (priceResponse.message && priceResponse.message.price) {
                            const priceElement = card.querySelector('.item-price');
                            if (priceElement) {
                                priceElement.textContent = `$${parseFloat(priceResponse.message.price).toFixed(2)}`;
                            }
                        }
                    } catch (itemError) {
                        console.log(`Could not update price for item ${itemCode}:`, itemError);
                    }
                }
            }
            
            console.log(`✅ Updated prices for ${itemCards.length} visible items`);
            
        } catch (error) {
            console.error('Error updating visible item prices:', error);
        }
    }
